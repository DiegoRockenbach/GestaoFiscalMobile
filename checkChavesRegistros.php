<?php
  include "inc/paginaRestrita.php";
	...
	if (isset($_POST['buscar'])) {
		...
		$chavesRegistros = $redis->keys("registros:*");
		rsort($chavesRegistros, SORT_REGULAR);
		
		foreach($chavesRegistros as $chave) {
			$mesChave = str_replace("registros:", "", $chave);
			...
			if (!empty($banco)) {
				foreach($banco as $registro) {
					...
					if (!empty($termoBusca)) {
						...
						$descricao = $registro->desc;
						
						foreach ($palavras as $palavra) {
							$palavra = trim($palavra);
							if (!empty($palavra) && stripos($descricao, $palavra) === false) {
								$match = false;
								break;
							}
						}
					...
					if ($valorMin !== '' && $registro->valor < floatval($valorMin)) {
						$match = false;
					}
					
					if ($valorMax !== '' && $registro->valor > floatval($valorMax)) {
						$match = false;
					}
					
					if ($match) {
						$registro->mesOrigem = $mesChave;
						$resultadosBusca[] = $registro;
						$somaTotal += floatval($registro->valor);
					}
				}
			}
		}
...
<!doctype html>
  <head>
  	...
  	<title>Checar chaves de registros</title>
		...
		<div class="searchBar">
			<form method="post" id="mainSearchForm" class="searchBarForm">
				<div class="searchInputGroup">
					<input type="text" name="termo" id="searchInput" placeholder="Buscar em todos os registros..." value="<?php echo htmlspecialchars($_POST['termo'] ?? ''); ?>" autocomplete="off">
  		...
  		<?php
			if ($buscaAtiva) {
				...
				if (empty($resultadosBusca)) {
					...
					echo "<div class='alert alert-warning nenhum_Registro'>Nenhum registro encontrado com os critérios especificados.</div>";
					echo "</div>";
				} else {
					echo "<div class='row mx-auto col-lg-10'>";
					echo "<div class='alert searchResultInfo'>";
					echo "Encontrados <strong>" . count($resultadosBusca) . "</strong> registro(s)";
					echo "</div>";
					...
					foreach($resultadosBusca as $row) {
						...
						echo "<td><a href='deleteRegistro.php?idGET=$row->id&dataGET=$dataFormatada'><img class='lixeiraIMG' src='inc/img/lixeira.png'></a></td>";
						echo "</tr>";
					}
					echo "</tbody>";
					echo "</table>";
				...
				$chavesRegistros = $redis->keys("registros:*");
				foreach($chavesRegistros as $row) {
				...
				echo "<tr>";
				echo "<th>Chaves de Registros</th>";
				echo "</tr></thead>";
				foreach($chavesRegistros as $row) {
					$row = str_replace(
						"registros:",
					...
					echo "<tr data-href='checkRegistros.php?rediskeyGET=registros:$row'>";
					echo "<td>$row</td>";
					echo "</tr>";
				}
				echo "</table>";
